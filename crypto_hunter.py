...
# ==========================================
# 2. CSS "BÊ TÔNG CỐT THÉP" (FIX CỨNG GIAO DIỆN)
# ==========================================
st.markdown("""
<style>
    /* Import Font */
    ...
    /* 2. 🔥 FIX Ô NHẬP LIỆU (INPUT) - QUAN TRỌNG NHẤT 🔥 */
    /* Ép nền của ô nhập liệu thành màu xám đậm */
    ...
    /* Ép màu chữ bên trong ô input thành màu TRẮNG */
    input[type="text"] {
        color: #eaecef !important;
        background-color: transparent !important; /* Để lộ nền xám bên dưới */
        caret-color: #fcd535 !important; /* Con trỏ nhấp nháy màu vàng */
    }
    
    /* Chỉnh màu chữ placeholder (chữ mờ gợi ý) */
    input::placeholder {
        color: #5e6673 !important;
    }

    /* 3. FIX Ô CHỌN (SELECTBOX) & MENU */
    ...
    /* Ép chữ trong nút thành màu ĐEN TUYỀN để dễ đọc */
    button[kind="primary"] * {
    ...
    h1, h2, h3, h4, span, p, label, .stMarkdown, div[data-testid="stCaptionContainer"] {
        color: #eaecef !important;
    }
    ...
    /* Xóa viền đỏ khi nhập sai (nếu có) */
...
if app_mode == "📈 MARKET DASHBOARD":
    ...
    with col_search:
        st.markdown("<small>🔍 NHẬP MÃ (VD: PEPE)</small>", unsafe_allow_html=True)
        # Placeholder sáng màu hơn để dễ đọc
    ...
    with st.spinner(f"Đang tải {symbol}..."):
        ...
        if df_backend.empty and "/USDT" in symbol:
             ...
             df_backend = fetch_candle_data_backend(fallback, '1h', 50)
    ...
    if not df_backend.empty:
        ...
        with m1: st.markdown(f"""<div class="binance-card"><div style="color:#848e9c;font-size:12px;">GIÁ HIỆN TẠI</div><div style="font-size:24px;font-weight:bold;" class="{color_class}">{curr['close']:,.4f}</div></div>""", unsafe_allow_html=True)
        with m2: st.markdown(f"""<div class="binance-card"><div style="color:#848e9c;font-size:12px;">THAY ĐỔI 1H</div><div style="font-size:24px;font-weight:bold;" class="{color_class}">{change_pct:+.2f}%</div></div>""", unsafe_allow_html=True)
        
        rsi = curr['RSI_14']
        rsi_col = "up-green" if rsi < 30 else ("down-red" if rsi > 70 else "#eaecef")
        with m3: st.markdown(f"""<div class="binance-card"><div style="color:#848e9c;font-size:12px;">RSI (14)</div><div style="font-size:24px;font-weight:bold;color:{rsi_col}">{rsi:.1f}</div></div>""", unsafe_allow_html=True)
        with m4: st.markdown(f"""<div class="binance-card"><div style="color:#848e9c;font-size:12px;">VOL (Nến cuối)</div><div style="font-size:24px;font-weight:bold;color:#eaecef">{curr['volume']:,.0f}</div></div>""", unsafe_allow_html=True)

        st.write("")
        render_tradingview_widget(symbol)
    else:
        st.warning(f"⚠️ Dữ liệu backend chưa cập nhật cho {symbol}.")
        render_tradingview_widget(symbol)
    ...
    if st.button("BẮT ĐẦU QUÉT NGAY", type="primary"):
        ...
        if results:
            ...
            st.info("Chưa có tín hiệu mạnh trong Top 30.")
